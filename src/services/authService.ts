...
export class AuthService {
    ...
    public async login(email: string, password: string): Promise<ApiResponse<{ token: string; user: User }>> {
        const response = await api.post('/auth/login', { email, password }) as ApiResponse<{ token: string; user: User }>;
    ...
    public async register(name: string, email: string, password: string): Promise<ApiResponse<{ token: string; user: User }>> {
        const response = await api.post('/auth/register', { name, email, password }) as ApiResponse<{ token: string; user: User }>;
        if (response.data?.token) {
            localStorage.setItem('token', response.data.token);
        }
        return response;
